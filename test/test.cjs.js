...
test('extract numbers and words and remove stopword numbers and korean words from korean text ', t => {
  ...
  const oldString =
    '쾰른 대성당(독일어: Kölner Dom, 정식 명칭: Hohe Domkirche St. Peter)은 독일 쾰른에 있는 로마 가톨릭교회의 성당이다. 고딕 양식으로 지어졌다. 쾰른 대교구의 주교좌 성당이라 쾰른 주교좌 성당이라고도 불린다. 현재 쾰른 대교구의 교구장은 라이너 마리아 뵐키 추기경이다. 이 성당은 독일에서 가장 잘 알려진 건축물로, 성 바실리 대성당에 이어, 1996년 유네스코 세계유산으로 등재되었다. 유네스코에서는 쾰른 대성당을 일컬어 “인류의 창조적 재능을 보여주는 드문 작품”이라고 묘사하였다.[1] 매일 2만여 명의 관광객이 이 성당을 찾는다.[2]'
  let newString = extract(oldString, {
    regex: [words, numbers],
    toLowercase: true
  })
...
test('remove english, swedish and custom stopwords', t => {
  t.plan(1)
  const oldString =
    'a really interesting string with some words trädgårdsägare är beredda att pröva vad som helst för att bli av med de hatade mördarsniglarna'.split(
      ' '
    )
...
test('should remove swedish stopwords and preserve case', t => {
  t.plan(1)
  const oldString =
    'Trädgårdsägare är beredda att pröva vad som helst för att bli av med de hatade mördarsniglarna åäö'.split(
      ' '
    )
...
test('remove danish stopwords', t => {
  t.plan(1)
  const oldString =
    'gæsterne i musikhuset i aarhus bør fremover sende en venlig tanke til den afdøde købmand herman salling når de skal til koncert eller se teater i det aarhusianske kulturhus æøå'.split(
      ' '
    )
...
test('remove hindu stopwords', t => {
  t.plan(1)
  const oldString =
    'केंद्र सरकार पर्यावरण के माकूल घरों ग्रीन होम्स को बढ़ावा देने की दिशा में गंभीरता से सोच रही है। ग्रीन हाउसिंग सोसायटी डिवेलप करने के लिए सरकार सस्ते लोन'.split(
      ' '
    )
...
test('remove spanish stopwords', t => {
  t.plan(1)
  const oldString =
    'los investigadores han analizado el adn de los restos de unos 200 gatos tomados de momias egipcias yacimientos vikingos y cuevas de la edad de piedra entre otros lugares variopintos'.split(
      ' '
    )
...
test('remove japanese stopwords after being tokenised', t => {
  ...
  t.plan(1)
  const oldString =
    '今 回作っ た リスト で は 校正 待ち と なっ て いる 作品 を 抽出 し 作者 や 作品 名 で の 検索 の ほか 作品 の 長さ さまざまな 理由 で 機械 的 に 処理 でき なかっ た もの は に なっ て しまっ て い ます が やいつ から 校正 待ち に なっ て いる か で 並べ替え が できる よう に し まし た'.split(
      ' '
    )
...
test('remove french stopwords', t => {
  t.plan(1)
  const oldString =
    'personne à commencer par theresa may n’aurait jamais imaginé une atmosphère aussi plombée un accablement collectif aussi manifeste pour ce qui devait être un jour de gloire'.split(
      ' '
    )
...
test('remove italian stopwords', t => {
  t.plan(1)
  const oldString =
    'la distruzione del califfato è ancora in corso ma già si è aperta la nuova fase nel duello fra stati uniti e russia in medio oriente'.split(
      ' '
    )
...
test('remove dutch stopwords', t => {
  t.plan(1)
  const oldString =
    'ludovici denkt dat het best kan werken als de gemeente werkloze jongeren aan hen koppelt natuurlijk kunnen ze ook rechtstreeks bij ons aankloppen aldus ludovici'.split(
      ' '
    )
...
test('remove polish stopwords', t => {
  t.plan(1)
  const oldString =
    'obecnie zatrzymali już sprzedawcę dopalaczy niespełna letniego chłopaka który usłyszy zarzuty sprzedaży nielegalnych substancji a tym samym narażania życia i zdrowia pokrzywdzonych w tej sprawie jest osób'.split(
      ' '
    )
...
test('remove portuguese stopwords', t => {
  t.plan(1)
  const oldString =
    'já são conhecidas as identidades de algumas das vítimas do incêndio de pedrógão grande em leiria que matou pessoas esta segunda-feira a embaixada de frança divulgou um comunicado onde confirma que um cidadão francês morreu no fogo de pedrógão'.split(
      ' '
    )
...
test('remove chinese simplified stopwords', t => {
  ...
  t.plan(1)
  const oldString =
    '起火 森林 位于 葡萄牙 中部 大 佩 德 罗 冈 市 火灾 发生 在 当地时间 日 时 左右 火势 迅速 向 四周 蔓延 酿成 重大 火灾 夜 之间 在 全国 范围 内 发生 了 大约 起 森林 大火 据 葡萄牙 新闻 报 报道 火灾 已 蔓延 至 莱 里 亚 大 区 的 多个 城镇 围绕 火 源 四 个 方向 的 火势 均 未 得到 控制 其中 有 两面 的 火 情 十分 严峻 伤亡 人数 很 可能 继续 增加'.split(
      ' '
    )
...
test('remove brazilian portuguese stopwords', t => {
  t.plan(1)
  const oldString =
    'agora já são conhecidas as identidades de algumas das vítimas do incêndio de pedrógão grande em leiria que matou pessoas esta segunda-feira a embaixada de frança divulgou um comunicado onde confirma que um cidadão francês morreu no fogo de pedrógão'.split(
      ' '
    )
...
test('remove punjabi gurmukhi stopwords', t => {
  t.plan(1)
  const oldString =
    'ਅਫਗਾਨੀਸਤਾਨ ਦੇ ਲਈ ਇੰਟਰਨੈੱਟ ਦਾ ਟਾੱਪ-ਲੈੱਵਲ ਡੋਮੇਨ ਦੇਸ਼ ਕੋਡ ਹੈ ਇਹ ਡੋਮੇਨ ਏ ਐਫ ਜੀ ਐਨ ਆਈ ਸੀ ਦੇ ਦੁਆਰਾ ਚਲਾਇਆ ਜਾਂਦਾ ਹੈ, ਜੋ ਅਫਗਾਨੀ ਸਰਕਾਰ ਅਤੇ ਯੂਨਾਈਟਡ ਨੇਸ਼ਨਜ਼ ਦੇ ਥੱਲੇ ਆਂਦਾ ਹੈ'.split(
      ' '
    )
...
test('remove swahili stopwords', t => {
  t.plan(1)
  const oldString =
    'kila mtu anaweza kuhariri makala yoyote kutoa makosa ya lugha kutohoa maneno na kuendeleza na kukuza makala kwa kuandika kwa ufupi au kwa urefu'.split(
      ' '
    )
...
test('remove finnish stopwords', t => {
  t.plan(1)
  const oldString =
    'vuonna 1502 kristoffer kolumbus saapui hondurasin rannikolle ja antoi sille sen nykyisen nimen'.split(
      ' '
    )
...
test('remove yoruba stopwords', t => {
  t.plan(1)
  const oldString =
    'jẹ́ ẹ̀gbà ọrùn tí wọ́n gbẹ́ lére tí ó sì jẹ́ àwòrán akọni obìrin tí a mọ̀ si ìyá wa olorì idia ti ọ̀rundún mẹ́rìndínlógún ṣẹ́yìn'.split(
      ' '
    )
...
test('remove zulu stopwords', t => {
  t.plan(1)
  const oldString =
    'ukhisimusi isikhathi esiletha injabulo kubantu abaningi emhlabeni jikelele lesi sikhathi senza ukuba kube khona ukuhlangana nemindeni okudala yagcinana kuhlalwe kuphunyulwe futhi kuncokolwe'.split(
      ' '
    )
...
test('remove sotho stopwords', t => {
  t.plan(1)
  const oldString =
    'ke boloetse ba batho bo bokwang ke kokwanahloko ya ebola matshwao hantlentle a qala matsatsi a mabedi ho isa ho a mararo ka mora ho tshwaetswa ke kokwanahloko'.split(
      ' '
    )
...
test('remove somali stopwords', t => {
  t.plan(1)
  const oldString =
    'isku celcelis qaarada antarktika waa tan ugu qaboow qalalsan ee ugu dabaysha badan qaaradaha caalamka oo dhan'.split(
      ' '
    )
...
test('remove vietnamese stopwords', t => {
  t.plan(1)
  const oldString =
    'Đà Lạt luôn hiện lên như một thành phố nghỉ dưỡng miền núi kiểu mẫu với cảnh quan thiên nhiên tươi đẹp'.split(
      ' '
    )
...
test('remove lugbara (without diacritics) stopwords', t => {
  t.plan(1)
  const oldString =
    "ri kome izu ni ma dria ri enya nya kabele izu ni si ku I vile akuri ra mu a'a oce ambu ni ma talaa a'a di idri ru eli izu".split(
      ' '
    )
...
test('remove lugbara official (with diacritics) stopwords', t => {
  t.plan(1)
  const oldString =
    "rɨ kómé èzʉ́ ni mà drìá, rɨ nyaká nyá kà'bèlé èzʉ́ nɨ sɨ ̀ Kʉ 'ɨ ́ vɨ ́lé àkú rɨ ̀ rá mu a'á òce àzɨ ́ àmbú nɨ mà táláá a'á élɨ ́ èzʉ́".split(
      ' '
    )
...
test('remove hausa stopwords', t => {
  t.plan(1)
  const oldString =
    'zaku iya rubutawa da ingantawa ko kuma ƙirƙiran sabbin muƙaloli kamar yadda kuke gani dan taimako wurin rubuta kundin Ilimi na Insakulofidiya wadda ke taskance ilimi dan masu karatu da yin bincike a harshen Hausa'.split(
      ' '
    )
...
test('remove afrikaans stopwords', t => {
  t.plan(1)
  const oldString =
    'alhoewel die oop see sowat een uur per motor van Bremen af lê is die getye op die Weserrivier twee keer per dag duidelik waarneembaar'.split(
      ' '
    )
...
test('remove indonesian stopwords', t => {
  t.plan(1)
  const oldString =
    'seorang anak perempuan di Bantul Yogyakarta sedang memasak nasi dengan bantuan tungku'.split(
      ' '
    )
...
test('remove armenian stopwords', t => {
  t.plan(1)
  const oldString =
    'Հայաստանի Հանրապետությունը ԱՀԿ անդամ է 1994 թվականի օգոստոսից և այդ ժամանակից էլ Հայաստանում ստեղծվել է ԱՀԿ-յան ներկայացուցչություն'.split(
      ' '
    )
...
test('remove basque stopwords', t => {
  t.plan(1)
  const oldString =
    'Garaipenek eta horien aberastasunek aldaketa sakonak eragin zituzten Erromako gizartean barnan Noblezia aberastu zen probintzien lepotik Soldadu xumeek ordea urte asko eman behar izaten zuten etxetik kanpoko borroketan'.split(
      ' '
    )
...
test('remove breton stopwords', t => {
  t.plan(1)
  const oldString =
    "gallout a ra pep hini ac’hanomp klask sevel ar pennadoù a vank dimp mankout a ra dimp ivez pennadoù hir n'hon eus nemet 24 evit ar mare".split(
      ' '
    )
...
test('remove bulgarian stopwords', t => {
  t.plan(1)
  const oldString =
    'когато генерал Жуков е назначен за командир на Киевския военен окръг в Украйна Баграмян му пише писмо в което го моли да бъде назначен под неговото командване'.split(
      ' '
    )
...
test('remove catalan stopwords', t => {
  t.plan(1)
  const oldString =
    'el candidat proposat exposa davant el Congrés dels Diputats el programa polític del Govern que pretengui formar i demana la confiança de la cambra'.split(
      ' '
    )
...
test('remove croatian stopwords', t => {
  t.plan(1)
  const oldString =
    'stanovnici priobalja su bili poznati po brodograditeljskom i pomorskom umijeću poznati po gradnji brodova i gusarenju bili su Liburni male i brze lađe kojima su plovili zvale su se lembi'.split(
      ' '
    )
...
test('remove czech stopwords', t => {
  t.plan(1)
  const oldString =
    'ptakopysk má zploštělý zobák který je pokryt zrohovatělou ale citlivou kůží ježury mají válcovitý rypec oběma typům čenich slouží jak k vyhledávání potravy tak k určení místa kde se nachází'.split(
      ' '
    )
...
test('remove esperanto stopwords', t => {
  t.plan(1)
  const oldString =
    'li mortiĝis kiam li estis 66jaraĝa en 1778 kaj liaj restoj estos metitaj en la Panteono fare de la Nacia Konvencio dum la Franca revolucio en 1794'.split(
      ' '
    )
...
test('remove estonian stopwords', t => {
  t.plan(1)
  const oldString =
    'nad hakkavad puitu auke puurima kuid sealt hakkab vaiku välja voolama ja esimesed üraskid tavaliselt hukkuvad selles kui puu on nõrga tervisega siis tema vastupanuvõime ammendub peatselt ja siis suudavad üraskid end puu sisse närida'.split(
      ' '
    )
...
test('remove galician stopwords', t => {
  t.plan(1)
  const oldString =
    'as máis antigas consistían nunha simple torre cadrada con muros anchos e cara o século XI adquiriran xa as características típicas a súa forma e función mudaron co tempo e variaban coa rexión'.split(
      ' '
    )
...
test('remove greek stopwords', t => {
  t.plan(1)
  const oldString =
    'eίναι σημαντικό ωστόσο ότι δεν υπάρχει καμία ομοφωνία στη χρονολογία του ψηφίσματος οι περισσότεροι ιστορικοί το τοποθετούν στο 433 άλλοι στο 432 και κάποιοι επιμένουν ότι μπορεί και να είναι πολύ μεταγενέστερο πιθανόν επειδή υπήρξαν και άλλα αντιμεγαρικά ψηφίσματα των Αθηναίων με αποτέλεσμα οι αρχαίοι ιστορικοί να τα συγχέουν'.split(
      ' '
    )
...
test('remove irish stopwords', t => {
  t.plan(1)
  const oldString =
    'ní raibh siad chomh drochamhrasach i leith na seanscéalta Págánacha agus a shílfeá ach mar sin féin bhain siad gach tagairt nithiúil don chreideamh réamh-Chríostaí de na leaganacha a rinne siad de na scéalta seo'.split(
      ' '
    )
...
test('remove korean stopwords', t => {
  t.plan(1)
  const oldString =
    '이 대회 결승전은 축구 자체보다 125,000명 정도 되는 웸블리 스타디움의 수용인원을 훌쩍 뛰어넘은 엄청난 인파로 인한 큰 혼란 때문에 더 중요하게 여겨진다 추정 300,000여 명의 관중들이 입장하면서 경기장의 테라스까지 꽉 차서 사람들이 넘칠 정도였고 관중들은 선수들이 경기하는 경기장 안 피치까지 내려가기 까지 했다'.split(
      ' '
    )
...
test('remove latvian stopwords', t => {
  t.plan(1)
  const oldString =
    'darbs patentu birojā deva ar zinātni nesaistītus ienākumus Einšteins vienmēr esot uzskatījis ka praktiska ar zinātni nesaistīta nodarbošanās atbrīvo zinātnieku no rūpēm par iztiku un no nepieciešamības publicēties tikai pašas publicitātes pēc'.split(
      ' '
    )
...
test('remove romanian stopwords', t => {
  t.plan(1)
  const oldString =
    'o sursă importantă a infecției în România sunt persoanele venite din afară care nu s-au autoizolat la domiciliu sau care au mințit că nu au fost într-o zonă afectată de infecția COVID-19'.split(
      ' '
    )
...
test('remove slovak stopwords', t => {
  t.plan(1)
  const oldString =
    'o zhruba desať mesiacov neskôr zbadali dvaja lesní robotníci popoludní 18. septembra 1916 pramienok vytekajúci z telesa hrádze'.split(
      ' '
    )
...
test('remove slovenian stopwords', t => {
  t.plan(1)
  const oldString =
    'Beta Andromede je rdeča orjakinja in je le za kanček temnejša kot Alfa njena barva pa je prepoznavna celo s prostim očesom dajbolj izrazito telo globokega vesolja je zagotovo Andromedina galaksija M31 tudi Velika galaksija v Andromedi ki je vidna celo s prostim očesom'.split(
      ' '
    )
...
test('remove turkish stopwords', t => {
  t.plan(1)
  const oldString =
    'bu anlaşmaya göre ABD askerleri 30 Haziran 2009 tarihinde Irak şehir merkezlerinden muharip güçlerini çekti[45] ve 30 Haziran Irak ta resmi tatil ilan edildi ancak anlaşma dahilinde 2009 yılını ortalarında bir halk referandumu yapılarak ABD güçlerinin konumu tekrar değerlendirildi'.split(
      ' '
    )
...
test('remove ukrainian stopwords', t => {
  t.plan(1)
  const oldString =
    '24 лютого 2022 року Росія відкрито напала на Україну. Військова кампанія почалася після тривалого військового нарощування, визнання Росією терористичних квазідержав «ДНР» та «ЛНР» як державних утворень 21 лютого. Практично одночасно ЗС РФ почали неприховане перекидання військ на окуповані території Донбасу.'
  const newString = removeStopwords(
    extract(oldString, { regex: [words], toLowercase: true }),
    ukr
  )
  t.deepEqual(newString, [
    'лютого',
...
test('remove malay stopwords', t => {
  t.plan(1)
  const oldString =
    'Ada juga orang ter-kenang-kan hal itu di hadap-an sahaya. Che Ismail sudah siap handak me-nikaḥ-kan anak-nia dengan Haji Daud.'
  const newString = removeStopwords(
    extract(oldString, { regex: [words], toLowercase: true }),
    msa
  )
...
test('remove gujarati stopwords', t => {
  t.plan(1)
  const oldString =
...
test('remove farsi stopwords and preserve case', t => {
  t.plan(1)
  const oldString =
    'در این بیانیه آمده است که اتو قادر به صحبت کردن، قادر به دیدن و قادر به عکس العمل نشان دادن به درخواست های شفاهی نبود'.split(
      ' '
    )
